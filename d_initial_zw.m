...
%  Set input/output NetCDF files.
 ...
 my_root = '/home/server/pi/homes/zwallace/ROMS/Project_Patagonia_Stretching';

 GRDname = fullfile(my_root, 'roms_grd_rivers.nc');
 OAname  = fullfile(my_root, 'Data/OA',      'oa4_lev94_feb.nc');
 INIname = fullfile(my_root, 'roms_ini_1900_new_stretching.nc');
...
S.ocean_time = 37064.0*86400;             % initial conditions time (s)

%--------------------------------------------------------------------------
%  Inquire dimensions and attritbutes from grid file.
%--------------------------------------------------------------------------
ninfo = nc_inq(GRDname);
S.Dimensions = ninfo.Dimensions;
S.Attributes = ninfo.Attributes;
S.Variables  = ninfo.Variables;
...
%[~]=c_initial(S);
[~]=nc_create(INIname,'NETCDF4',S);
  
% avalue='365.25 days in every year';
